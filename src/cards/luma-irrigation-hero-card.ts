...
export class LumaIrrigationHeroCard extends LitElement implements LovelaceCard {
  ...
  static styles=[lumaTokens,css`
    ...
    .chips{display:flex;flex-wrap:wrap;gap:8px;margin-top:15px}.chip{padding:7px 11px;border-radius:999px;color:var(--tone);background:color-mix(in srgb,var(--tone) 11%,transparent);font-size:11px;font-weight:700}.next{display:inline-flex;align-items:center;gap:7px;border:0;cursor:pointer;font:inherit}.next ha-icon{--mdc-icon-size:15px}
  ...
  setConfig(c:Config){if(!c?.controller_entity||!c.zones)throw Error("controller_entity and zones required");this.config={name:"Öntözési központ",schedule_path:"/dashboard-irrigation/schedules",schedules:[],...c}}
  ...
  render(){
    ...
    const s=this.hass.states,c=this.config,online=s[c.controller_entity]?.state==="on",rain=s[c.rain_entity]?.state==="on",paused=s[c.suspended_entity]?.state==="on",message=c.message_entity&&s[c.message_entity]?.state==="on",active=c.zones.filter(x=>s[x]?.state==="on"),state=s[c.program_entity]?.state||"Nincs adat",program=c.active_program_entity?s[c.active_program_entity]?.state||"Nincs":"Nincs",programActive=!['','nincs','none','unknown','unavailable'].includes(program.toLocaleLowerCase()),running=active.length>0||programActive,progress=Math.max(0,Math.min(100,Number(c.progress_entity?s[c.progress_entity]?.state:NaN))),remaining=Number(c.remaining_entity?s[c.remaining_entity]?.state:NaN),nextZone=c.next_zone_entity?s[c.next_zone_entity]?.state:"",next=this.nextSchedule();
    ...
    const activeName=active.length===1?entityName(s[active[0]],active[0]).replace(/^irrigation-controller\s*/i,""):active.length?`${active.length} aktív zóna`:"",runLabel=programActive?[program,activeName||nextZone].filter(x=>x&&!['Nincs','-1'].includes(x)).join(" · "):activeName||state,subtitle=!online?"A vezérlő nem elérhető":running?runLabel:paused?"Az automatikus indítás felfüggesztve":rain?"Eső miatti tiltás":state;
    return html`<ha-card style=${`--tone:${tone}`}><div class="top"><span class="icon"><ha-icon icon=${icon}></ha-icon></span><span class="copy"><div class="name">${c.name}</div><div class="sub">${subtitle}</div></span></div>${running&&Number.isFinite(progress)?html`<div class="progress"><span class="progress-copy">${runLabel||"Öntözés folyamatban"}</span><span class="remaining">${Number.isFinite(remaining)&&remaining>0?`${Math.ceil(remaining)} perc hátra`:`${Math.round(progress)}%`}</span><div class="track"><div class="fill" style=${`width:${progress}%`}></div></div></div>`:nothing}<div class="chips"><span class="chip">${online?"Vezérlő online":"Offline"}</span><span class="chip">${rain?"Eső érzékelve":"Száraz"}</span>${running?html`<span class="chip">${programActive?program:`${active.length} zóna fut`}</span>`:nothing}${paused?html`<span class="chip">Felfüggesztve</span>`:nothing}${next?html`<button class="chip next" @click=${()=>runAction(this,this.hass!,{action:"navigate",navigation_path:c.schedule_path!})}><ha-icon icon="mdi:calendar-clock"></ha-icon>Következő: ${this.scheduleLabel(next)}</button>`:html`<span class="chip">Nincs ütemezett futás</span>`}</div></ha-card>`;
  }
}
