...
const stateLabelsHu:Record<string,string>={
  open:"Nyitva",opening:"Nyílik",closed:"Zárva",closing:"Záródik",
  on:"Nyitva",off:"Zárva",unavailable:"Nem elérhető",unknown:"Ismeretlen",
};
const stateLabelsEn:Record<string,string>={open:"Open",opening:"Opening",closed:"Closed",closing:"Closing",on:"Open",off:"Closed",unavailable:"Unavailable",unknown:"Unknown"};
...
export class LumaGateCard extends LitElement implements LovelaceCard {
  ...
  static styles=[lumaTokens,css`
    ...
    @media(max-width:420px){.compact .summary{grid-template-columns:38px minmax(0,1fr);grid-template-areas:"gate-icon gate-copy" "gate-icon gate-status";gap:3px 10px;padding:13px}.compact .icon{grid-area:gate-icon;align-self:center;width:38px;height:38px;border-radius:13px}.compact .copy{grid-area:gate-copy}.compact .title{white-space:normal;line-height:1.15}.compact .hint{margin-top:2px}.compact .status{grid-area:gate-status;justify-self:start;margin:2px 0 0;padding:4px 7px;font-size:9px}.popover{padding:13px}.popover .actions{display:grid;grid-template-columns:minmax(0,1fr) auto}.popover .action{padding:0 12px}}
  ...
  render(){
    ...
    const primary=moving
      ? {id:"stop",label:localize(this.hass,"stop"),icon:"mdi:stop",domain:"cover",service:"stop_cover"}
      : cover
        ? open
          ? {id:"close",label:localize(this.hass,"close"),icon:"mdi:gate",domain:"cover",service:"close_cover"}
          : {id:"open",label:localize(this.hass,"open"),icon:"mdi:gate-open",domain:"cover",service:"open_cover"}
        : {id:"toggle",label:open?localize(this.hass,"close"):localize(this.hass,"open"),icon:open?"mdi:garage":"mdi:garage-open",domain:"switch",service:"toggle"};
    ...
    const button=(id:string,label:string,buttonIcon:string,domain:string,service:string,target=c.entity,primaryButton=false,direct=false)=>{
      ...
      return html`<button class=${`action ${primaryButton?"primary":""} ${confirming?"confirm":""}`} ?disabled=${unavailable} @click=${()=>direct?this.execute(domain,service,target):this.act(id,domain,service,target)}><ha-icon icon=${confirming?"mdi:check":buttonIcon}></ha-icon><span>${confirming?localize(this.hass,"confirm"):label}</span></button>`;
    };
    if(c.display==="popover")return html`
      <ha-card class="compact" style=${`--tone:${tone}`}>
        <button class="summary" aria-haspopup="dialog" aria-expanded=${this.popoverOpen} @click=${()=>this.toggleActionPopover()}>
          ...
          <span class="copy"><span class="title">${c.name||localized(this.hass,"Gate","Kapu")}</span><span class="hint">${localize(this.hass,"tap_to_control")}</span></span>
          <span class="status"><span class="dot"></span>${localizedMap(this.hass,stateLabelsEn,stateLabelsHu,s)}</span>
        </button>
      </ha-card>
      ${this.popoverOpen?html`<button class="scrim" aria-label=${localize(this.hass,"dismiss")} @click=${()=>this.toggleActionPopover()}></button><section class="popover" style=${`--tone:${tone}`} role="dialog" aria-label=${`${c.name||localized(this.hass,"Gate","Kapu")} ${localized(this.hass,"controls","vezérlés")}`}>
        <div class="popover-head"><span class="icon"><ha-icon icon=${icon}></ha-icon></span><div><div class="popover-title">${c.name||localized(this.hass,"Gate","Kapu")}</div><div class="popover-state">${localizedMap(this.hass,stateLabelsEn,stateLabelsHu,s)}</div></div><button class="close" aria-label=${localize(this.hass,"dismiss")} @click=${()=>this.toggleActionPopover()}><ha-icon icon="mdi:close"></ha-icon></button></div>
        <div class="actions">${button(primary.id,primary.label,primary.icon,primary.domain,primary.service,c.entity,true,true)}${c.pedestrian_entity?button("walk",localize(this.hass,"pedestrian"),"mdi:walk","button","press",c.pedestrian_entity,false,true):nothing}</div>
        <button class="details" @click=${()=>runAction(this,this.hass!,{action:"more-info"},c.state_entity||c.entity)}>${localize(this.hass,"history")}</button>
      </section>`:nothing}`;
    return html`<ha-card style=${`--tone:${tone}`}>
      <button class="summary" @click=${()=>runAction(this,this.hass!,{action:"more-info"},c.state_entity||c.entity)}>
        <span class="icon"><ha-icon icon=${icon}></ha-icon></span>
        <span class="copy"><span class="title">${c.name||localized(this.hass,"Gate","Kapu")}</span><span class="hint">${localize(this.hass,"tap_for_history")}</span></span>
        <span class="status"><span class="dot"></span>${localizedMap(this.hass,stateLabelsEn,stateLabelsHu,s)}</span>
      </button>
      <div class="actions">
        ${button(primary.id,primary.label,primary.icon,primary.domain,primary.service,c.entity,true)}
        ${c.pedestrian_entity?button("walk",localize(this.hass,"pedestrian"),"mdi:walk","button","press",c.pedestrian_entity):nothing}
      </div>
    </ha-card>`;
  }
}
