...
export class LumaUpdateCard extends LitElement implements LovelaceCard {
  ...
  render(){if(!this.hass||!this.config)return nothing;const entity=this.hass.states[this.config.entity],a=entity?.attributes||{},raw=a.in_progress,installing=raw===true||(typeof raw==="number"&&raw>=0),percentage=Number(a.update_percentage??(typeof raw==="number"?raw:NaN)),progress=Number.isFinite(percentage)?Math.max(0,Math.min(100,percentage)):undefined,installed=String(a.installed_version||""),latest=String(a.latest_version||""),available=entity?.state==="on",tone=installing?"var(--info-color,var(--primary-color))":available?"var(--warning-color)":"var(--success-color)",subtitle=installing?(progress!==undefined?`Telepítés · ${Math.round(progress)}%`:"Telepítés folyamatban"):available?(installed&&latest?`${installed} → ${latest}`:"Frissítés elérhető"):"Naprakész";return html`<ha-card class=${installing?"installing":""} style=${`--tone:${tone}`} @click=${()=>runAction(this,this.hass!,{action:"more-info"},this.config!.entity)}><div class="row"><span class="icon"><ha-icon icon=${installing?"mdi:progress-download":this.config.icon||"mdi:package-up"}></ha-icon></span><span><div class="name">${this.config.name||entityName(entity,this.config.entity)}</div><div class="sub">${subtitle}</div></span>${available||installing?html`<button class="action" ?disabled=${installing} @click=${this.install}>${installing?(progress!==undefined?`${Math.round(progress)}%`:"TELEPÍTÉS"):this.pending?"MEGERŐSÍTÉS":"TELEPÍTÉS"}</button>`:nothing}</div>${installing?html`<div class="track"><div class=${`fill ${progress===undefined?"indeterminate":""}`} style=${progress===undefined?"":`width:${progress}%`}></div></div>`:nothing}</ha-card>`}
}
