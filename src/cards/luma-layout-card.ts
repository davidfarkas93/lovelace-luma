...
interface Config{type:string;cards:Record<string,unknown>[];columns?:number;tablet_columns?:number;mobile_columns?:number;gap?:number;tablet_breakpoint?:number;mobile_breakpoint?:number}
...
@customElement("luma-layout-card")export class LumaLayoutCard extends LitElement implements LovelaceCard{
 ...
 static styles=[lumaTokens,css`:host{display:block}.grid{display:grid;grid-template-columns:repeat(var(--desktop),minmax(0,1fr));gap:var(--gap)}@media(max-width:1023px){.grid{grid-template-columns:repeat(var(--tablet),minmax(0,1fr))}}@media(max-width:599px){.grid{grid-template-columns:repeat(var(--mobile),minmax(0,1fr))}}`];
 ...
 render(){if(!this.config)return nothing;return html`<div class="grid" style=${`--desktop:${this.config.columns};--tablet:${this.config.tablet_columns};--mobile:${this.config.mobile_columns};--gap:${this.config.gap}px`}><span></span></div>`}
}
