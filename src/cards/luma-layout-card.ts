...
@customElement("luma-layout-card")export class LumaLayoutCard extends LitElement implements LovelaceCard{
 ...
 static styles=[lumaTokens,css`:host{display:block;min-width:0}.grid{display:grid;grid-template-columns:var(--desktop-template,repeat(var(--desktop),minmax(0,1fr)));align-items:start;gap:var(--gap);min-width:0}.grid>*{min-width:0;max-width:100%}@media(max-width:1023px){.grid{grid-template-columns:repeat(var(--tablet),minmax(0,1fr))}}@media(max-width:599px){.grid{grid-template-columns:repeat(var(--mobile),minmax(0,1fr))}}`];
