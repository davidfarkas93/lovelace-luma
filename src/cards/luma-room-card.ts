...
export class LumaRoomCard extends LitElement implements LovelaceCard {
  ...
  private legacyItems(c:Config):LumaEntityItem[]{const out:LumaEntityItem[]=[];if(c.light_entity)out.push({entity:c.light_entity,name:"Fény",icon:"mdi:lightbulb-outline",show_state:true,tap_action:{action:"toggle"}});if(c.cover_entity)out.push({entity:c.cover_entity,name:"Redőny",icon:"mdi:window-shutter",show_state:true,tap_action:{action:"more-info"}});return out}
  ...
  render(){
    ...
    const environment=c.subtitle||[t==null?"":`${t.toFixed(1)}°C`,h==null?"":`${Math.round(h)}% pára`].filter(Boolean).join(" · ")||"Részletek és vezérlés";
