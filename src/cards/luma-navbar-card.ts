...
export class LumaNavbarCard extends LitElement implements LovelaceCard{
  ...
  static styles=css`
    ...
    .backdrop{position:absolute;z-index:-2;inset:-24px;width:calc(100% + 48px);height:calc(100% + 48px);object-fit:cover;filter:blur(24px) saturate(1.32);opacity:.28;transform:scale(1.1)}.media-dock.has-art::after{content:"";position:absolute;z-index:-1;inset:0;background:linear-gradient(100deg,color-mix(in srgb,var(--card-background-color) 86%,transparent) 4%,color-mix(in srgb,var(--card-background-color) 68%,transparent) 58%,color-mix(in srgb,var(--card-background-color) 78%,transparent));box-shadow:inset 0 1px 0 color-mix(in srgb,#fff 10%,transparent)}
