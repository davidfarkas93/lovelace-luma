...
export class LumaLogbookCard extends LitElement implements LovelaceCard {
  ...
  private relative(d:Date){const m=Math.max(0,Math.floor((Date.now()-d.getTime())/60000));if(m<1)return"épp most";if(m<60)return`${m} perce`;const h=Math.floor(m/60);if(h<24)return`${h} órája`;return`${Math.floor(h/24)} napja`}
  render(){if(!this.config)return nothing;const limit=this.expanded?this.events.length:(this.config.collapse||5),shown=this.events.slice(0,limit),groups=new Map<string,LogbookEvent[]>();for(const e of shown){const key=this.date(e).toLocaleDateString(this.hass?.locale?.language||"hu-HU",{year:"numeric",month:"long",day:"numeric"});groups.set(key,[...(groups.get(key)||[]),e])}return html`<ha-card>${this.config.title?html`<div class="title"><ha-icon icon=${this.config.icon}></ha-icon>${this.config.title}</div>`:nothing}${this.loading&&!this.events.length?html`<div class="empty">Előzmények betöltése…</div>`:!shown.length?html`<div class="empty">${this.config.no_event||"Nincs esemény a kiválasztott időszakban."}</div>`:[...groups].map(([day,events])=>html`<div class="day">${day}</div>${events.map(e=>html`<div class="event"><span class="event-icon"><ha-icon icon=${e.icon||this.config!.icon}></ha-icon></span><span class="message">${e.message||e.name||e.state||"Öntözési esemény"}</span><span class="time">${this.relative(this.date(e))}</span></div>`)}`)}${this.events.length>(this.config.collapse||5)?html`<button class="more" @click=${()=>this.expanded=!this.expanded}><ha-icon icon=${this.expanded?"mdi:chevron-up":"mdi:chevron-down"}></ha-icon>${this.expanded?"Kevesebb":"További események"}</button>`:nothing}</ha-card>`}
}
