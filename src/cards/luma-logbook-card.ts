...
interface Config { type:string; entity?:string; entities?:string[]; hours_to_show?:number; max_items?:number; collapse?:number; no_event?:string; icon?:string; title?:string; deduplicate_seconds?:number }
...
export class LumaLogbookCard extends LitElement implements LovelaceCard {
  ...
  setConfig(c:Config){if(!c?.entity&&!c?.entities?.length)throw Error("entity or entities required");this.config={hours_to_show:168,max_items:20,collapse:5,icon:"mdi:water-check-outline",deduplicate_seconds:5,...c};this.loaded=false}
  ...
  private async load(){if(!this.hass?.callWS||!this.config)return;this.loading=true;const end=new Date(),start=new Date(end.getTime()-(this.config.hours_to_show||168)*3600000),ids=this.config.entities||[this.config.entity!];try{const result=await this.hass.callWS<LogbookEvent[]>({type:"logbook/get_events",start_time:start.toISOString(),end_time:end.toISOString(),entity_ids:ids});const events=(Array.isArray(result)?result:[]).slice().reverse();this.events=this.deduplicate(events).slice(0,this.config.max_items)}catch{this.events=[]}finally{this.loading=false}}
  private eventText(e:LogbookEvent){return String(e.message||e.name||e.state||"").trim()}
  private deduplicate(events:LogbookEvent[]){const seconds=Math.max(0,this.config?.deduplicate_seconds??5);if(!seconds)return events;const windowMs=seconds*1000,seen=new Map<string,number>();return events.filter(e=>{const key=this.eventText(e),when=this.date(e).getTime(),previous=seen.get(key);seen.set(key,when);return !key||previous===undefined||Math.abs(previous-when)>windowMs})}
