...
export class LumaHomelabHeroCard extends LitElement implements LovelaceCard {
  ...
  private summary(){
    ...
    if(kuma)parts.push(`${kuma} Kuma`);if(komodo)parts.push(`${komodo} Komodo`);if(critical+warning)parts.push(`${critical+warning} infrastruktúra`);
    return{total,tone:kuma+critical>0?"var(--error-color)":total>0?"var(--warning-color)":"var(--success-color)",subtitle:total?`${parts.join(" · ")} figyelmet kér`:`${urls.length} szolgáltatás felügyelve · minden rendszer működik`};
  }
  render(){if(!this.hass||!this.config)return nothing;const status=this.summary();return html`<ha-card class="interactive" style=${`--tone:${status.tone}`} @click=${()=>runAction(this,this.hass!,this.config?.tap_action)}><span class="icon"><ha-icon icon=${this.config.icon||"mdi:server-security"}></ha-icon></span><span class="title">${this.config.name||"Homelab Control Center"}</span><span class="subtitle">${status.subtitle}</span><span class="badge">${status.total?`${status.total} incidens`:"RENDBEN"}</span></ha-card>`}
}
