...
const weatherLabelsHu: Record<string, string> = {
  sunny: "Napos",
  "clear-night": "Derült éjszaka",
  cloudy: "Felhős",
...
const weatherLabelsEn:Record<string,string>={sunny:"Sunny","clear-night":"Clear night",cloudy:"Cloudy",partlycloudy:"Partly cloudy",rainy:"Rainy",pouring:"Pouring rain",lightning:"Thunderstorms","lightning-rainy":"Thunderstorms with rain",snowy:"Snowy",fog:"Fog",windy:"Windy"};
...
export class LumaHomeHeroCard extends LitElement implements LovelaceCard {
  ...
  static styles = [
    ...
    css`
      .hero {
        ...
        overflow: hidden;
        border: 1px solid
          color-mix(in srgb, var(--luma-accent) 20%, transparent);
        border-radius: var(--luma-radius-hero);
        background: linear-gradient(
          135deg,
          color-mix(in srgb, var(--luma-accent) 16%, var(--luma-surface)),
          color-mix(in srgb, var(--luma-accent) 3%, var(--luma-surface)) 68%
        );
      ...
      .hero.interactive:hover {
        ...
        box-shadow: 0 23px 58px rgba(0, 0, 0, 0.11);
      }
      .content {
        position: relative;
        z-index: 2;
      ...
      .top {
        ...
        grid-template-areas: "icon title status" "icon subtitle status";
        align-items: center;
        gap: 0 17px;
      }
      .weather-icon {
      ...
      .attention {
        ...
        background: color-mix(in srgb, var(--attention-color) 12%, transparent);
      }
      .active {
        position: relative;
        overflow: visible;
      ...
      .alarm-popover {
        ...
        border: 1px solid color-mix(in srgb, var(--alarm-tone) 20%, transparent);
        border-radius: 22px;
        background: color-mix(in srgb, var(--luma-surface) 94%, transparent);
        box-shadow:
          0 24px 70px rgba(0, 0, 0, 0.22),
          0 4px 16px color-mix(in srgb, var(--alarm-tone) 10%, transparent);
        backdrop-filter: blur(22px) saturate(1.25);
        animation: alarm-appear 0.18s cubic-bezier(0.2, 0.8, 0.2, 1);
      ...
      .alarm-popover-icon {
        ...
        height: 38px;
        border-radius: 13px;
        color: var(--alarm-tone);
        background: color-mix(in srgb, var(--alarm-tone) 15%, transparent);
      }
      .alarm-popover-icon ha-icon {
        --mdc-icon-size: 20px;
      }
      .alarm-popover-title {
        font-size: var(--luma-text-md);
        font-weight: var(--luma-weight-title);
      }
      .alarm-popover-state {
        margin-top: 2px;
        color: var(--alarm-tone);
        font-size: var(--luma-text-xs);
        font-weight: 680;
      }
      .alarm-close {
        display: grid;
      ...
      .alarm-actions {
        display: grid;
        grid-template-columns: repeat(var(--alarm-count), minmax(0, 1fr));
        gap: 5px;
        padding: 5px;
        border-radius: 999px;
        background: color-mix(
          in srgb,
          var(--primary-text-color) 4.5%,
          transparent
        );
      }
      .alarm-mode {
      ...
      .alarm-mode:hover:not(:disabled) {
        background: color-mix(in srgb, var(--mode-tone) 10%, transparent);
      }
      .alarm-mode.confirm {
      ...
      .issue {
        ...
        background: color-mix(in srgb, var(--issue-color) 8%, transparent);
      }
      .issue ha-icon {
        --mdc-icon-size: 17px;
      }
      ...
      .sun {
        background: radial-gradient(
          circle,
          #e9ad35 0 31%,
          color-mix(in srgb, #ffd36a 34%, transparent) 33% 53%,
          transparent 55%
        );
      ...
      .moon {
        ...
        right: 24%;
        background: transparent;
        box-shadow: -25px 18px 0 color-mix(in srgb, #aab6ff 32%, transparent);
      }
      .cloud {
        position: absolute;
        right: 20%;
      ...
      .rotor {
        ...
        animation: spin var(--wind-duration, 8s) linear infinite;
      ...
      @media (max-width: 900px) {
        .top {
          ...
          grid-template-areas: "icon title" "icon subtitle" "status status";
          gap: 0 14px;
        }
        .status {
  ...
  protected updated(): void {
    ...
    this.renderRoot
      .querySelector(".panel")
      ?.addEventListener("click", this.stopIncidentClick);
  }
  ...
  disconnectedCallback() {
    ...
    this.detachAlarmListeners();
  }

  private toggleAlarmPopover() {
    this.alarmPopoverOpen = !this.alarmPopoverOpen;
    this.alarmPending = undefined;
    if (this.alarmPopoverOpen) {
      window.addEventListener("resize", this.repositionAlarm);
      window.addEventListener("scroll", this.repositionAlarm, true);
      void this.updateComplete.then(() => this.positionAlarmPopover());
    } else this.detachAlarmListeners();
  }
  private detachAlarmListeners() {
    window.removeEventListener("resize", this.repositionAlarm);
    window.removeEventListener("scroll", this.repositionAlarm, true);
  }
  private positionAlarmPopover() {
    if (!this.alarmPopoverOpen) return;
    const anchor = this.renderRoot.querySelector<HTMLElement>(".alarm"),
      popover = this.renderRoot.querySelector<HTMLElement>(".alarm-popover");
    if (!anchor || !popover) return;
    const rect = anchor.getBoundingClientRect(),
      gap = 8,
      pad = 12,
      width = Math.min(370, window.innerWidth - pad * 2),
      left = Math.min(
      ...
      top =
        below + popover.offsetHeight <= window.innerHeight - pad
          ? below
          : Math.max(pad, rect.top - popover.offsetHeight - gap);
  ...
  private activateAlarm(
    ...
    if (!this.hass || !this.config?.alarm_entity || disabled) return;
    if (this.alarmPending !== mode) {
      this.alarmPending = mode;
    ...
    this.alarmPending = undefined;
    this.alarmPopoverOpen = false;
    this.detachAlarmListeners();
    void this.hass.callService(
      "alarm_control_panel",
      mode === "disarm" ? "alarm_disarm" : `alarm_arm_${mode}`,
      undefined,
  ...
  private get watchedIds(): string[] {
    ...
    const fixed = [
      ...
      this.config.alarm_entity,
      this.config.notifications_entity,
      this.config.acknowledgements_entity,
      this.config.irrigation_entity,
      ...(this.config.irrigation_zone_entities || []).map((item) =>
        typeof item === "string" ? item : item.entity,
      ),
      this.config.waste_entity,
      this.config.waste_ack_entity,
      ...(this.config.waste_items || []).map((item) => item.entity),
      ...(this.config.appliances || []).flatMap((item) => [
        item.entity,
        item.active_entity,
    ...
    const active = Object.keys(this.hass.states).filter((id) =>
      ["light", "media_player", "climate"].includes(id.split(".")[0]),
    );
    return [
      ...new Set([
        ...(fixed.filter(Boolean) as string[]),
        ...dynamic,
        ...related,
        ...active,
        ...(this.config.banners || []).map((b) => b.entity),
      ]),
    ];
  }
  ...
  private greeting(): string {
    const h = new Date().getHours(),
      user = this.config?.name || this.hass?.user?.name || "",
      first = user.trim().split(/\s+/)[0],
      name = first ? `, ${first}` : "";
    return `${h >= 18 ? localized(this.hass,"Good evening","Jó estét") : h >= 12 ? localized(this.hass,"Good afternoon","Szép délutánt") : h >= 5 ? localized(this.hass,"Good morning","Jó reggelt") : localized(this.hass,"Hello","Szia")}${name}!`;
  }
  private wasteSummary(): string {
    if (!this.hass || !this.config) return "";
    const items = (this.config.waste_items || [])
      .map((item) => ({
        name: item.name,
        days: Number(this.hass!.states[item.entity]?.attributes.daysTo),
      }))
    ...
    const days = items[0].days,
      names = items
        ...
        .map((item) => item.name)
        .join(" + "),
      when = days === 0 ? localized(this.hass,"Today","Ma") : days === 1 ? localized(this.hass,"Tomorrow","Holnap") : localized(this.hass,`In ${days} days`,`${days} nap múlva`);
    return `${when} · ${names}`;
  }
  ...
  private irrigationSummary(): string {
    if (!this.hass || !this.config) return "";
    const program = this.config.irrigation_entity
      ? this.hass.states[this.config.irrigation_entity]?.state || ""
      : "";
    if (
      program &&
      !listed(["Nincs", "none", "unknown", "unavailable", ""], program)
    )
      return program;
    const active = (this.config.irrigation_zone_entities || []).filter(
      (item) =>
        this.hass!.states[typeof item === "string" ? item : item.entity]
          ?.state === "on",
    );
    return active
      .map((item) =>
        typeof item === "string"
          ? String(this.hass!.states[item]?.attributes.friendly_name || item)
          : item.name ||
            String(
  ...
  private accent(
    ...
    if (["lightning", "lightning-rainy", "pouring"].includes(state))
      return "var(--error-color)";
    if (["rainy", "snowy", "snowy-rainy", "fog"].includes(state))
      return "var(--info-color, var(--primary-color))";
  ...
  private ackMap(): Record<string, number> {
    const raw = this.config?.acknowledgements_entity
      ? this.hass?.states[this.config.acknowledgements_entity]?.state || ""
      : "";
    const result: Record<string, number> = {};
    for (const item of raw.split(",")) {
      const p = item.lastIndexOf(":");
      if (p > 0) result[item.slice(0, p)] = Number(item.slice(p + 1)) || 0;
    }
    return result;
  }
  ...
  private async dismiss(
    ...
    await this.hass.callService(
      ...
      "set_value",
      { value },
      { entity_id: this.config.acknowledgements_entity },
    );
  }
  ...
  private renderWeatherFx(state: string, wind: number) {
    ...
    const sky = [
      ...
      : state === "clear-night" || h < 6 || h >= 20
        ? html`<div class="moon"></div>`
        : html`<div class="sun"></div>`;
    const turbine =
      wind >= (this.config?.wind_threshold || 8)
        ? html`<div
            class="wind"
            style=${`--wind-duration:${Math.max(2.8, 14 - wind * 0.45)}s`}
          >
            <div class="mast"></div>
            <div class="rotor">
              <i class="blade"></i><i class="blade"></i><i class="blade"></i
              ><i class="hub"></i>
            </div>
          </div>`
        : nothing;
  ...
  private renderBanner(item: LumaBannerConfig) {
    ...
    const entity = this.hass.states[item.entity],
      color = item.color || "var(--luma-accent)";
    return html`<div
    ...
    >
      <ha-icon icon=${item.icon || "mdi:information-outline"}></ha-icon
      ><span class="banner-label">${item.label || item.name}</span
      ><span class="banner-state"
        >${item.state_label ||
        entityState(this.hass, entity, item.state_map)}</span
      ><button
        class="banner-action"
        @click=${(e: Event) => {
          e.stopPropagation();
          void runAction(
            this,
            this.hass!,
            item.secondary_action || item.tap_action,
            item.entity,
          );
        }}
      >
        ${item.secondary_label || item.name || localize(this.hass, "open_item")}
      </button>
    </div>`;
  }
  ...
  render() {
    ...
    const weather = this.hass.states[this.config.weather_entity],
      ...
      wind = Number(a.wind_speed) || 0;
    const issues = this.incidents(),
      critical = issues.some((x) => x.tone === "error"),
      attentionColor = critical
        ? "var(--error-color)"
        : issues.length
          ? "var(--warning-color)"
          : "var(--success-color)",
      active = this.activeCount(),
      accent = this.accent(state, Number(a.temperature), issues);
    const subtitle = [
      localizedMap(this.hass,weatherLabelsEn,weatherLabelsHu,state),
      a.temperature != null
        ? `${a.temperature} ${a.temperature_unit || "°C"}`
        : "",
      wind ? `${localized(this.hass,"wind","szél")} ${wind} ${a.wind_speed_unit || "km/h"}` : "",
      issues.length
        ? localized(this.hass,`${issues.length} warnings`,`${issues.length} figyelmeztetés`)
        : localized(this.hass,"all systems operational","minden rendszer rendben"),
    ]
      .filter(Boolean)
      .join(" • ");
    const alarm = this.config.alarm_entity
        ? this.hass.states[this.config.alarm_entity]
        : undefined,
      armed =
        alarm && !["disarmed", "unknown", "unavailable"].includes(alarm.state),
    ...
    if (irrigation)
      banners.push({
        entity:
          this.config.irrigation_entity ||
          String(
            typeof this.config.irrigation_zone_entities?.[0] === "string"
              ? this.config.irrigation_zone_entities[0]
    ...
    if (
      this.config.waste_entity &&
      this.hass.states[this.config.waste_ack_entity || ""]?.state !== "on"
    )
      banners.push({
        entity: this.config.waste_entity,
        label: localized(this.hass,"Waste","Hulladék"),
        state_label: this.wasteSummary() || undefined,
        name: localize(this.hass, "open_item"),
        ...
        secondary_action: this.config.waste_ack_entity
          ? {
              ...
              target: { entity_id: this.config.waste_ack_entity },
            }
          : undefined,
      });
    for (const item of this.config.appliances || []) {
      const appliance = this.hass.states[item.entity],
        state = appliance?.state.toLowerCase() || "unavailable",
        idle = item.idle_states || [
          "off",
          "power_off",
          "idle",
          "end",
        ...
        active = item.active_entity
          ? this.hass.states[item.active_entity]?.state === "on"
          : !idle.includes(state),
        remainingEntity = item.remaining_entity
          ? this.hass.states[item.remaining_entity]
          : undefined,
        remaining = applianceSeconds(remainingEntity),
        finish = applianceFinishTime(remainingEntity);
      if (active)
        banners.push({
          entity: item.entity,
          label: item.name,
          state_label: [
            entityState(this.hass, appliance, item.state_map),
            remaining != null ? `${applianceDuration(remaining)} ${localized(this.hass,"left","hátra")}` : "",
            finish ? `${localized(this.hass,"done","kész")} ${finish}` : "",
          ]
            .filter(Boolean)
            .join(" · "),
          name: localize(this.hass, "open_item"),
    ...
    const alarmModes = this.config.alarm_modes || [
      {mode:"away",name:localized(this.hass,"Away","Távol"),icon:"mdi:shield-lock"},
      {mode:"night",name:localized(this.hass,"Night","Éjszaka"),icon:"mdi:shield-moon"},
      {mode:"disarm",name:localized(this.hass,"Disarm","Kikapcsolás"),icon:"mdi:shield-off-outline"},
    ],
      alarmUnavailable =
        !alarm || ["unknown", "unavailable"].includes(alarm.state),
      alarmStateMap: Record<string, string> = {
        away: "armed_away",
        night: "armed_night",
        home: "armed_home",
        disarm: "disarmed",
      },
      alarmModeIcons: Record<string, string> = {
        ...
        <div class="content">
          <div class="top">
            <div class="weather-icon">
              <ha-icon
                icon=${weatherIcons[state] || "mdi:home-heart"}
              ></ha-icon>
            </div>
            <h2>${this.greeting()}</h2>
            ...
            <div class="status">
              <button
                ...
                @click=${(e: Event) => {
                  e.stopPropagation();
                  void runAction(this, this.hass!, this.config?.active_action);
                }}
              >
                <ha-icon icon="mdi:lightning-bolt-outline"></ha-icon
                ><span>${localized(this.hass,"Active","Aktív")}</span>${active > 0
                  ? html`<span class="active-badge">${active}</span>`
                  : nothing}</button
              >${alarm
                ? html`<button
                    class="alarm interactive"
                    aria-haspopup=${this.config.alarm_popover
                      ? "dialog"
                      : "false"}
                    aria-expanded=${this.alarmPopoverOpen}
                    style=${`--alarm-color:${alarmColor}`}
                    @click=${(e: Event) => {
                      e.stopPropagation();
                      this.config?.alarm_popover
                        ? this.toggleAlarmPopover()
                        : void runAction(
                            this,
                            this.hass!,
                            this.config?.alarm_action || {
                              action: "more-info",
                            },
                            this.config?.alarm_entity,
                          );
                  ...
                  >
                    ...
                    ><span>${entityState(this.hass, alarm)}</span>
                  </button>`
                : nothing}<button
                class="attention interactive"
                style=${`--attention-color:${attentionColor}`}
                @click=${(e: Event) => {
                  e.stopPropagation();
              ...
              >
                ...
                ><span
                  >${issues.length
                    ? localized(this.hass,`${issues.length} notices`,`${issues.length} jelzés`)
                    : localized(this.hass,"All good","Rendben")}</span
                >
              </button>
            </div>
          </div>
          ${this.detailsOpen && issues.length
            ? html`<div class="panel">
                ${issues.map(
                  (issue) =>
                    html`<div
                      class="issue"
                      style=${`--issue-color:${issue.tone === "error" ? "var(--error-color)" : "var(--warning-color)"}`}
                    >
                      <ha-icon
                        icon=${issue.tone === "error"
                      ...
                      >${issue.dismissible && issue.tone !== "error"
                        ? html`<span class="issue-actions"
                            ><button
                              @click=${(e: Event) => this.dismiss(issue, 7, e)}
                            >
                              ${localized(this.hass,"7 days","7 nap")}</button
                            ><button
                              @click=${(e: Event) => this.dismiss(issue, 30, e)}
                            >
                              ${localized(this.hass,"30 days","30 nap")}
                            </button></span
                          >`
                        : nothing}
                    </div>`,
            ...
            : nothing}${banners.some((b) => itemIsVisible(this.hass!, b))
            ? html`<div class="banners">
                ${banners.map((b) => this.renderBanner(b))}
              </div>`
            : nothing}
        </div></ha-card
      >${this.alarmPopoverOpen && alarm
        ? html`<button
              class="alarm-scrim"
              aria-label=${localize(this.hass, "dismiss")}
              @click=${() => this.toggleAlarmPopover()}
            ></button>
            <section
              class="alarm-popover"
              style=${`--alarm-tone:${alarmColor}`}
              role="dialog"
              aria-label=${localized(this.hass,"Alarm controls","Riasztó vezérlés")}
            >
              <div class="alarm-popover-head">
                <span class="alarm-popover-icon"
                  ><ha-icon icon="mdi:shield-home-outline"></ha-icon
                ></span>
                <div>
                  <div class="alarm-popover-title">${localized(this.hass,"Alarm","Riasztó")}</div>
                  <div class="alarm-popover-state">
                    ${entityState(this.hass, alarm)}
                  </div>
                </div>
                <button
                  class="alarm-close"
              ...
              <div
                class="alarm-actions"
                style=${`--alarm-count:${alarmModes.length}`}
              >
                ${alarmModes.map((mode) => {
                  const active = alarm.state === alarmStateMap[mode.mode],
                    transitioning = ["arming", "pending"].includes(alarm.state),
                    disabled =
                      active ||
                      alarmUnavailable ||
                      ((alarm.state === "triggered" || transitioning) &&
                        mode.mode !== "disarm"),
                    confirming = this.alarmPending === mode.mode,
                    tone =
                      mode.mode === "disarm"
                        ? "var(--error-color)"
                        : mode.mode === "night"
                          ...
                          : "var(--success-color)";
                  return html`<button
                    class=${`alarm-mode ${active ? "active" : ""} ${confirming ? "confirm" : ""}`}
                    style=${`--mode-tone:${tone}`}
                    ?disabled=${disabled}
                    @click=${() => this.activateAlarm(mode.mode, disabled)}
                  >
                    <ha-icon
                      icon=${confirming
                        ? "mdi:check"
                        : mode.icon || alarmModeIcons[mode.mode]}
                    ></ha-icon
                    ><span>${confirming ? localize(this.hass, "confirm") : mode.name}</span>
                  </button>`;
                })}
              </div>
              <button
                ...
                @click=${() => {
                  ...
                  void runAction(
                    this,
                    this.hass!,
                    this.config?.alarm_action || { action: "more-info" },
                    this.config?.alarm_entity,
                  );
                }}
              >
