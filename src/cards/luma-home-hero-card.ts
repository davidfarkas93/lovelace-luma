...
interface LumaHomeHeroConfig {
  ...
  appliances?: Array<{entity:string;name:string;remaining_entity?:string;path?:string;icon?:string;idle_states?:string[];state_map?:Record<string,string>}>;
...
export class LumaHomeHeroCard extends LitElement implements LovelaceCard {
  ...
  private get watchedIds(): string[] {
    ...
    const fixed = [this.config.weather_entity,this.config.alarm_entity,this.config.notifications_entity,this.config.acknowledgements_entity,this.config.irrigation_entity,...(this.config.irrigation_zone_entities||[]).map(item=>typeof item==="string"?item:item.entity),this.config.waste_entity,this.config.waste_ack_entity,...(this.config.waste_items||[]).map(item=>item.entity),...(this.config.appliances||[]).flatMap(item=>[item.entity,item.remaining_entity])];
  ...
  render(){
    ...
    for(const item of this.config.appliances||[]){const appliance=this.hass.states[item.entity],state=appliance?.state.toLowerCase()||"unavailable",idle=item.idle_states||["off","power_off","idle","end","complete","completed","unknown","unavailable"],remainingEntity=item.remaining_entity?this.hass.states[item.remaining_entity]:undefined,remaining=applianceSeconds(remainingEntity),finish=applianceFinishTime(remainingEntity);if(!idle.includes(state))banners.push({entity:item.entity,label:item.name,state_label:[entityState(this.hass,appliance,item.state_map),remaining!=null?`${applianceDuration(remaining)} hátra`:"",finish?`kész ${finish}`:""].filter(Boolean).join(" · "),name:"MEGNYITÁS",icon:item.icon||"mdi:washing-machine",color:"var(--info-color, var(--primary-color))",tap_action:{action:"navigate",navigation_path:item.path||"/dashboard-rooms/laundry"}})}
