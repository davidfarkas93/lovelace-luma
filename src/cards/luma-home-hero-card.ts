...
export class LumaHomeHeroCard extends LitElement implements LovelaceCard {
  ...
  static styles = [lumaTokens, css`
    ...
    .banner-action { padding:4px 7px; border-radius:999px; background:color-mix(in srgb,var(--item-color) 14%,transparent); font-size:9px; font-weight:800; letter-spacing:.05em; }
  ...
  private renderBanner(item:LumaBannerConfig){
    ...
    return html`<button class="banner interactive" style=${`--item-color:${color}`} @click=${(e:Event)=>{e.stopPropagation();void runAction(this,this.hass!,item.tap_action,item.entity)}}><ha-icon icon=${item.icon||"mdi:information-outline"}></ha-icon><span class="banner-label">${item.label||item.name}</span><span class="banner-state">${item.state_label||entityState(this.hass,entity,item.state_map)}</span><span class="banner-action">${item.name||"MEGNYITÁS"}</span></button>`;
  }
