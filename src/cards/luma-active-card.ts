...
export class LumaActiveCard extends LitElement implements LovelaceCard {
  ...
  render() {
    ...
    return html`<div class="wrap">
      <div class="heading"><ha-icon icon="mdi:lightning-bolt-outline"></ha-icon><span class="heading-title">${this.config.name || localized(this.hass,"Currently active","Most aktív")}</span><span class="count">${items.length}</span>${activeLights.length ? html`<button class="all-off" ?disabled=${this.turningOff} title=${localize(this.hass,"all_lights_off")} @click=${() => this.turnOffAll(activeLights)}><ha-icon icon=${this.turningOff ? "mdi:loading" : "mdi:lightbulb-group-off-outline"}></ha-icon><span>${this.turningOff ? localize(this.hass,"turning_off") : localize(this.hass,"all_lights_off")}</span></button>` : nothing}</div>
      ${items.length ? html`<div class="grid">${repeat(items, ({ entity }) => entity.entity_id, ({ entity, rule }) => {
        ...
        const childConfig={type:"custom:luma-control-card",entity:entity.entity_id,subtitle:rule.area_name||entityAreaName(this.hass!,entity.entity_id),display_as:rule.display_as,embedded_details:true,tap_action:action,hold_action:{action:"more-info"}};
      ...
      })}</div>` : html`<div class="empty">${this.config.empty_text || localized(this.hass,"No active devices","Nincs aktív eszköz")}</div>`}
    </div>`;
  }
}
