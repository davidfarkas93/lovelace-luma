...
export class LumaWeatherForecastCard extends LitElement implements LovelaceCard {
  ...
  setConfig(config: Config) {
    ...
    this.config = { days: 7, locale: "hu-HU", ...config };
    this.fetchedFor = "";
    void this.load();
  }
  ...
  private windDirection(value?: number) {
    ...
    return ["É", "ÉK", "K", "DK", "D", "DNy", "Ny", "ÉNy"][Math.round(value / 45) % 8];
  }

  render() {
    if (!this.config) return nothing;
    if (this.loading) return html`<ha-card><div class="loading">Előrejelzés betöltése…</div></ha-card>`;
    if (!this.forecast.length) return html`<ha-card><div class="loading">Nincs elérhető előrejelzés</div></ha-card>`;
    const lang = this.config.locale || this.hass?.locale?.language || "hu-HU";
    ...
    return html`<ha-card><div class="days" style=${`--days:${this.forecast.length}`}>${this.forecast.map(item => {
      ...
      <div class="sheet-top"><span class="sheet-icon"><ha-icon icon=${icons[selected.condition || ""] || "mdi:weather-cloudy"}></ha-icon></span><span><div class="sheet-title">${selected.datetime ? new Intl.DateTimeFormat(lang,{weekday:"long",month:"long",day:"numeric"}).format(new Date(selected.datetime)) : "Részletek"}</div><div class="sheet-sub">${labels[selected.condition || ""] || selected.condition || "Időjárási előrejelzés"}</div></span><button class="close" @click=${() => this.selected = undefined}><ha-icon icon="mdi:close"></ha-icon></button></div>
      <div class="facts">${this.fact("mdi:thermometer-high","Maximum",selected.temperature,"°")}${this.fact("mdi:thermometer-low","Minimum",selected.templow,"°")}${this.fact("mdi:thermometer-lines","Hőérzet",selected.apparent_temperature,"°")}${this.fact("mdi:weather-pouring","Csapadék esélye",selected.precipitation_probability,"%")}${this.fact("mdi:water","Csapadék",selected.precipitation,` ${weatherAttrs.precipitation_unit || "mm"}`)}${this.fact("mdi:water-percent","Páratartalom",selected.humidity,"%")}${this.fact("mdi:weather-windy","Szél",selected.wind_speed,` ${weatherAttrs.wind_speed_unit || "km/h"}`)}${this.fact("mdi:windsock","Szélirány",this.windDirection(selected.wind_bearing))}${this.fact("mdi:weather-tornado","Széllökés",selected.wind_gust_speed,` ${weatherAttrs.wind_speed_unit || "km/h"}`)}${this.fact("mdi:gauge","Légnyomás",selected.pressure,` ${weatherAttrs.pressure_unit || "hPa"}`)}${this.fact("mdi:weather-sunny-alert","UV-index",selected.uv_index)}${this.fact("mdi:eye-outline","Látótávolság",selected.visibility,` ${weatherAttrs.visibility_unit || "km"}`)}</div>
      ${hours.length ? html`<div class="hourly-title">Napközbeni alakulás</div><div class="hourly">${hours.map(hour => html`<div class="hour"><time>${new Intl.DateTimeFormat(lang,{hour:"2-digit",minute:"2-digit"}).format(new Date(hour.datetime!))}</time><ha-icon icon=${icons[hour.condition || ""] || "mdi:weather-cloudy"}></ha-icon><span class="hour-temp">${hour.temperature ?? "—"}°</span>${hour.precipitation_probability != null ? html`<span class="hour-rain">${hour.precipitation_probability}%</span>` : nothing}</div>`)}</div>` : nothing}
    </section></div>` : nothing}`;
  }
}
