...
interface Config { type:string; entity:string; state_entity?:string; name?:string; subtitle?:string; subtitle_entity?:string; subtitle_state_map?:Record<string,string>; icon?:string; color?:string; active_color?:string; active_states?:string[]; state_map?:Record<string,string>; action_label?:string; display_as?:"light"|"media_player"|"switch"; artwork?:boolean; artwork_entity?:string; embedded_details?:boolean; compact?:boolean; wrap_value?:boolean; wrap_subtitle?:boolean; tap_action?:LumaAction; hold_action?:LumaAction }
...
export class LumaControlCard extends LitElement implements LovelaceCard {
  ...
  static styles=[lumaTokens,css`
    ...
    .card.wrap-value{grid-template-columns:46px minmax(0,1fr);grid-template-areas:"icon name" "icon subtitle" "value value"}.card.wrap-value .value{justify-self:stretch;margin-top:9px;padding:8px 11px;border-radius:13px;line-height:1.35;white-space:normal;overflow-wrap:anywhere;text-align:left}.card.wrap-value .action{display:none}
    ...
    @media(max-width:599px){.card{grid-template-columns:42px minmax(0,1fr) auto;grid-template-areas:"icon name value" "icon subtitle value";padding:13px}.card.wrap-value{grid-template-columns:42px minmax(0,1fr);grid-template-areas:"icon name" "icon subtitle" "value value"}.icon{width:42px;height:42px;border-radius:14px}.action{display:none}}
  `];
  setConfig(config:Config){if(!config?.entity)throw Error("Luma control requires entity.");this.config={active_states:[],...config}}
  getCardSize(){return 2}
  protected shouldUpdate(changed:PropertyValues<this>){if(!changed.has("hass"))return true;const old=changed.get("hass") as HomeAssistant|undefined;const entity=this.config?.entity||"",stateEntity=this.config?.state_entity||entity,subtitleEntity=this.config?.subtitle_entity||"",artwork=this.config?.artwork_entity||"";return !old||old.states[entity]!==this.hass?.states[entity]||old.states[stateEntity]!==this.hass?.states[stateEntity]||(subtitleEntity!==""&&old.states[subtitleEntity]!==this.hass?.states[subtitleEntity])||(artwork!==""&&old.states[artwork]!==this.hass?.states[artwork])}
  private startHold(){
    this.held=false;
  ...
  render(){
    ...
    const subtitleSource=this.config.subtitle_entity?this.hass.states[this.config.subtitle_entity]:undefined;
    const subtitle=subtitleSource?entityState(this.hass,subtitleSource,this.config.subtitle_state_map):this.config.subtitle||(domain==="light"?(area||localized(this.hass,"Not assigned to an area","Nincs helyiséghez rendelve")):domain==="media_player"&&active&&media?media:entityName(e,localized(this.hass,"State","Állapot")));
