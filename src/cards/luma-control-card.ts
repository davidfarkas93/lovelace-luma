...
export class LumaControlCard extends LitElement implements LovelaceCard {
  ...
  render(){if(!this.hass||!this.config)return nothing;const e=this.hass.states[this.config.entity],domain=this.config.entity.split(".")[0],defaults=domain==="light"?["on"]:domain==="media_player"?["on","playing","paused","buffering"]:[],states=this.config.active_states?.length?this.config.active_states:defaults,active=states.includes(e?.state),base=domain==="light"?"var(--warning-color)":domain==="media_player"?"#7c78d8":"var(--primary-color)",tone=active?(this.config.active_color||base):(this.config.color||base),brightness=Number(e?.attributes?.brightness),media=String(e?.attributes?.media_title||e?.attributes?.app_name||""),subtitle=this.config.subtitle||(domain==="light"&&Number.isFinite(brightness)?`${Math.round(brightness/255*100)}% fényerő`:domain==="media_player"&&media?media:entityName(e,"Állapot"));return html`<ha-card class=${`card ${this.config.tap_action?"interactive":""}`} style=${`--tone:${tone};--mix:${active?"13%":"4%"};--border-mix:${active?"22%":"10%"};--icon-mix:${active?"21%":"11%"};--pill-mix:${active?"16%":"7%"};--shadow-mix:${active?"12%":"4%"};--value-color:${active?tone:"var(--luma-muted)"}`} role=${this.config.tap_action?"button":"presentation"} tabindex=${this.config.tap_action?"0":"-1"} @pointerdown=${()=>this.startHold()} @pointerup=${()=>this.cancelHold()} @pointerleave=${()=>this.cancelHold()} @click=${()=>{if(!this.held)void runAction(this,this.hass!,this.config?.tap_action,this.config?.entity)}}><div class="icon"><ha-icon icon=${this.config.icon||entityIcon(e)}></ha-icon></div><div class="name">${this.config.name||entityName(e,this.config.entity)}</div><div class="subtitle">${subtitle}</div><div class="value">${entityState(this.hass,e,this.config.state_map)}</div>${this.config.action_label?html`<div class="action">${this.config.action_label}</div>`:nothing}</ha-card>`}
}
