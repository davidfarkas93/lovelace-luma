...
export class LumaControlCard extends LitElement implements LovelaceCard {
  ...
  render(){
    ...
    const brightness=Number(e?.attributes?.brightness),media=String(e?.attributes?.media_title||e?.attributes?.app_name||"");
    const subtitle=this.config.subtitle||(domain==="light"&&active&&Number.isFinite(brightness)?`${Math.round(brightness/255*100)}% fényerő`:domain==="media_player"&&active&&media?media:entityName(e,"Állapot"));
    ...
    return html`<ha-card class=${`card ${this.config.tap_action?"interactive":""}`} style=${vars} role=${this.config.tap_action?"button":"presentation"} tabindex=${this.config.tap_action?"0":"-1"} @pointerdown=${()=>this.startHold()} @pointerup=${()=>this.cancelHold()} @pointerleave=${()=>this.cancelHold()} @click=${()=>{if(!this.held)void runAction(this,this.hass!,this.config?.tap_action,this.config?.entity)}}><div class="icon"><ha-icon icon=${this.config.icon||entityIcon(e)}></ha-icon></div><div class="name">${this.config.name||entityName(e,this.config.entity)}</div><div class="subtitle">${subtitle}</div><div class="value">${entityState(this.hass,e,this.config.state_map)}</div>${this.config.action_label?html`<div class="action">${this.config.action_label}</div>`:nothing}</ha-card>`;
  }
}
