...
[![Validate](https://github.com/davidfarkas93/lovelace-luma/actions/workflows/validate.yml/badge.svg)](https://github.com/davidfarkas93/lovelace-luma/actions/workflows/validate.yml)
[![HACS](https://img.shields.io/badge/HACS-Custom-41BDF5.svg?logo=home-assistant-community-store)](https://hacs.xyz/)
[![Release](https://img.shields.io/github/v/release/davidfarkas93/lovelace-luma?display_name=tag&sort=semver)](https://github.com/davidfarkas93/lovelace-luma/releases/latest)
[![Storybook](https://img.shields.io/badge/Storybook-Explore-FF4785?logo=storybook&logoColor=white)](https://davidfarkas93.github.io/lovelace-luma/)
[![License](https://img.shields.io/github/license/davidfarkas93/lovelace-luma)](LICENSE)

A polished, context-aware Lovelace card suite for Home Assistant.

![Luma home hero](docs/luma-home-hero.svg)

**[Explore every card in the interactive component catalogue](https://davidfarkas93.github.io/lovelace-luma/)** — including responsive viewports, working actions, live-style mock data, and a production-scale home hero pattern.
...
Built-in interface copy follows Home Assistant's active language. English is
the fallback locale and Hungarian is included; explicit labels supplied in a
card configuration always take precedence over the built-in translation. The
...
| `custom:luma-ev-control-card` | Compact swipeable charging and Solar Boost controls with inline confirmation |
...
| `custom:luma-irrigation-schedule-card` | Explicit weekday/every-N-days mode selector, persistent boolean mode, automation health and next-run preview |
...
| `custom:luma-discovery-card` | Native runtime discovery for favorite lights, Homelab incidents, infrastructure updates, and Komodo deployments |
...
`luma-metric-card` is the single compact read-only status primitive. It supports
numeric formatting, secondary entities, mapped states, and direct attribute
display through `attribute`. The former `luma-status-card` was removed in 0.54;
replace its type with `custom:luma-metric-card` without changing the remaining
configuration.
...
Luma includes a Storybook catalogue that renders the real Lit card components
against a safe, simulated Home Assistant runtime. Entity states and card options
can be changed from Storybook controls; service calls are captured in an action
log and never reach a Home Assistant instance.
